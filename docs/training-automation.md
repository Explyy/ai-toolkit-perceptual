...
Set the template command to `/run-unified-training`. `TRAINING_GUI_START` may name another existing executable when the inherited image does not use `/start.sh`; it is never parsed as a shell command. The image build verifies that the pinned base still provides executable `/start.sh`. The automation writes its actual state to `/storage/automation/unified/supervisor-state.json` and its log to `supervisor.log`. An error is recorded as `held`; the GUI stays reachable and the supervisor polls again. Persistent GUI sessions are never automatically deleted.

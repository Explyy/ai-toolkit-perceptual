...
Sample association reads AI Toolkit names like `TIMESTAMP__000000100_0.png`, includes the unnumbered final checkpoint at the configured final step, and accepts a verified backup receipt when a retained checkpoint has already been pruned locally. If two immutable remote weight variants share one job and step, the report keeps both candidates but marks remote association ambiguous because sample filenames cannot identify the bytes that produced them. Such metrics remain reviewable, but remote `select` refuses to guess a catalog checkpoint. If resumed sampling creates duplicates, evaluation selects the latest complete timestamp run and records how many older or partial samples it discarded. Clipping ranking is available only when every checkpoint has the same complete prompt/index/seed set. Face identity compares the intersection of prompt indices with one valid face at every complete checkpoint; at least three shared prompts are required. The automatic shortlist is available when every sample run is complete, the configured prompt/seed evidence is comparable, and at least one checkpoint has a unique verified remote association. A checkpoint whose association is ambiguous or absent cannot be exported, so it is excluded from the candidates and listed with its reason in `automatic_shortlist.excluded`; it no longer voids the ranking of the checkpoints that are uniquely associated, which is what made an extended run publish no top three at all while the rest of its own checkpoints were complete and verified. The shortlist is unavailable only when no checkpoint is uniquely associated, when the prompt/seed evidence is not comparable, or when the identity ranking itself is unavailable. It reports at most three evidence candidates ordered by mean face cosine descending, mean clipping ascending, then step ascending, records a partial-coverage reason when it excluded anything, and otherwise records an explicit unavailable reason. A partial ranking is visible where a human reads it and not only in the evaluation report: the published `index.json` carries `excluded_candidates` with each step, its association status and its reason, and the gallery prints the same list under the shortlist, so three candidates are never mistaken for the whole run. It never selects a checkpoint. These fixed-model CPU results are deterministic for the pinned code, inputs, and runtime, but are not guaranteed bitwise-identical across hardware and are not ground truth about quality or overtraining.
...
The automatic publisher uses the report's existing shortlist only after rechecking complete comparable prompt/seed sets, available aggregate rankings, and the deterministic top of the uniquely associated candidates. It recomputes that eligible subset itself, refuses a shortlist that is not exactly its top three, and refuses any shortlisted step without a unique verified association. It never makes a human selection or changes `selected_checkpoint_id`. For each available candidate, up to three, it downloads the checkpoint weight at the exact catalog revision, verifies the catalog size and SHA-256, and uploads a verified copy with its raster contact sheet and machine-readable evidence:
...
The opt-in `/run-parallel-training` command runs one explicitly assigned shard and replaces the inherited GUI command for that container. Set the SimplePod template `argOptions` field to exactly `/run-parallel-training`, as one argument without shell metacharacters. The public provider API no longer honours that field: `POST` and `PUT` on `/instances/templates` answer `200` and store `null`, so a template maintained through the API silently starts the inherited GUI instead of the worker, which is what emptied the 2026-09-15 launch. Set `argOptions` through the provider UI, or dispatch the worker from the per-instance `startScript`, which the API does persist. The same original template can be switched temporarily for the two disposable launches and restored to `/run-unified-training` afterward; no second template or execution-mode wrapper is required. It writes the complete child output to both the console and `<storage>/automation/<run>/<shard>/worker.log`. A failed child is held in the same container for diagnosis instead of exiting into a provider restart loop, but that hold is a deadline, not a state: when the hold starts, the worker prints the failure reason and the absolute UTC instant at which it ends, and at that instant the instance deletes itself through the same identity-verified lifecycle path as the success route. `TRAINING_FAILURE_HOLD_SECONDS` configures the window and defaults to 21600 seconds (six hours); `0` deletes as soon as the failure is recorded. The self-delete uses the instance binding the bootstrap verified and persisted in `bootstrap-state.json` before any paid work, and records the delete request durably before issuing it. Only a child that actually exited non-zero reaches that hold. The child's output is decoded as UTF-8 with undecodable bytes replaced, because the stream carries tqdm progress bars whose glyphs are multi-byte and a read boundary can fall inside one: strict decoding turned that ordinary byte into an exception on the very first progress bar of every fresh pod, which is how two healthy trainings were deleted 9 and 19 steps before their first checkpoint on 2026-09-18. If the supervisor loses the stream anyway, that loss is a fact about the supervisor and never about the child: nothing is signalled, the failed read is retried, and the supervisor then asks the child itself for its exit status, waiting for as long as the child is alive. The console says which of the two happened, so a lost pipe over a living child no longer reads as a dead shard. Only the status the child really returned can schedule the hold, the group termination and the self-delete; `exit=127` again means the child ended that way. When the child's outcome cannot be established at all - no readable stream and no answerable exit status - the supervisor exits `125`, holds nothing and deletes nothing, because unknown is not failure; it also says plainly what that means, since exiting ends the container command: the instance is not deleted, the child is not protected from a provider restart, and manual inspection is needed now. Both sinks of the console record, the container stdout and the durable log, are written independently and best-effort. This supervisor is the container command, so an exception from a full `/storage` volume or a log consumer that went away would end the container, take a still-training child with it and leave the instance billing with the hold and the self-delete unreachable - exactly the situation they exist for. Losing one sink never costs the line in the other, and a read that fails is retried before the stream is given up. Waiting on a child that never exits is the same unbounded wait a readable pipe already implies for a hung trainer; what is not allowed is converting a living child into a failure. A shard that did fail keeps the whole hold: an in-flight publication is never cut short by it. Death is established once, at the deadline, immediately before the delete request, and over every child the supervisor ever started, because an archive-only relaunch can leave an earlier one alive. It is established on the process group, not the pid. The supervised child is `python -m training_automation parallel-run`, which runs the trainer as its own subprocess; signalling the leader alone kills the queue and orphans a trainer that keeps writing. The child is therefore started with `start_new_session=True` and the whole group receives SIGTERM, then SIGKILL, and the group must be observed gone. A group that cannot be signalled or will not empty blocks the deletion entirely and asks for manual cleanup: deleting an instance whose trainer may still be publishing truncates evidence that costs GPU hours to reproduce. An unconfirmed deletion is not terminal either: the hold repeats up to three times, 1800 seconds apart, printing each attempt and its next deadline, so a provider outage spanning the inner retry budget cannot leave the instance billing forever. The repetition is classified, not blind. Waiting can change an unconfirmed answer, a transport failure, a 5xx or a local mount that refused one read of the persisted binding; it cannot change revoked credentials, a refused method, an instance whose identity does not match this process, or a binding that was never persisted. The first class is re-held inside the budget, the second fails at the first attempt, and a delete refused with one of those permanent statuses stops the inner budget too once the instance is confirmed still present. The overlay does not set a new `CMD` or `ENTRYPOINT`, so normal launches continue to run the original GUI. The selected queue recipe is [trainer-subject-likeness-masked-klein-9b.yaml](../config/examples/klein_automation/trainer-subject-likeness-masked-klein-9b.yaml), copied from `subject_likeness_masked_flux2_klein9b` at `origin/perceptual-port@ab5f146e9a30764a40bf29b0881d3a6dd7186c6f`. It retains batch size 4, AdamW8bit at `5e-5`, LoKr 32, relative weight noise `0.0125` with `bound_norm: true`, resolution repeats `[16,4,1]`, masked depth loss `0.005`, and subject weights `background=0`, `clothing=1`, `body=1`; DOP is enabled from the beginning with multiplier `1` and class `woman`. Fresh manifests supply each dataset's seven-epoch step count and accounting explicitly. The template's 1200-step value remains only the backward-compatible legacy fallback. Its six seed-pinned `subject-likeness-core-v3` prompts are saved and sampled every 100 steps.
...
`TRAINING_STORAGE_ROOT` defaults to `/storage`, `TRAINING_REPO_ROOT` defaults to `/app/ai-toolkit`, and `TRAINING_RECIPE_PATH` defaults to the provenance-locked recipe above. Set optional `TRAINING_LORAS_ROOT` to the existing shared ComfyUI LoRA directory when the disposable worker must install all three ranked results before shutdown. Optional `TRAINING_FAILURE_HOLD_SECONDS` bounds the diagnostic hold of a failed shard and defaults to six hours. Generated config, queue state, staging state, output, and archive state are isolated under `<storage>/{automation,output,datasets}/<run_id>/<shard_id>`. The generated dedicated-worker config disables the GUI logger and removes its SQLite path because this command does not start the GUI; normal GUI launches still inherit the original behavior. The manifest must assign exactly three datasets to each shard. Before paid work, bootstrap checks `storage.minimum_free_bytes`, verifies all expected numeric catalog IDs were reserved with their canonical Hub base model metadata, and stages every model source at its exact revision.

An optional top-level `selected_dataset_ids` field narrows the requested shard to specific jobs while retaining the same autonomous staging, queue, evaluation, publication, archive, and verified instance-deletion path. The manifest must still contain the complete, valid schema-1 cohort of six datasets across two three-job shards. The field must be a nonempty list of unique string IDs, and every ID must belong to `TRAINING_SHARD_ID`; unknown IDs and IDs from the other shard are refused. Selection follows dataset order in the manifest, regardless of the order of IDs in this field. Omitting the field preserves the existing three-job shard behavior. For a shard `b` run containing Losciuto, use:

```json
{
  "selected_dataset_ids": ["losciuto"]
}
```

...
2. Sync top-1 result pointers, scan the current local cache, and reconcile the private completion ledger. An explicit historical result seed is used only for a model that has no current latest pointer.

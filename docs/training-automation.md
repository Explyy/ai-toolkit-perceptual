...
The image contains InsightFace Buffalo L from the versioned upstream v0.7 release. Its remote build pins the 288,621,354-byte archive to SHA-256 `80ffe37d8a5940d59a7384c201a2a38d4741f2f3c51eef46ebb28218a7b0ca2f` and records the archive and individual model-file hashes in `/opt/training-automation-models/SOURCES.json`. The parallel manifest configures `InsightFaceCPUBackend` against that directory and requires available comparable reference identity before completion. These pretrained weights are restricted to noncommercial research unless separately licensed. Do not configure the pose backend for this deployment unless a compatible `.task` model and the documented MediaPipe dependency are also installed; an absent backend remains `unavailable` and never counts as identity success.

...
export interface NumberInputProps extends InputProps {
  ...
  min?: number;
  max?: number;
  // When true, fully clearing the field emits null so the consumer can drop
  // the config key (used by the per-dataset overrides, which inherit the
  // global value when their key is absent). `unsetOnEmpty` is the fork-era
  // alias for upstream's `allowEmpty`; either enables the behavior.
  allowEmpty?: boolean;
  unsetOnEmpty?: boolean;
}

export const NumberInput = (props: NumberInputProps) => {
  const { label, value, onChange, placeholder, required, min, max, docKey = null, unsetOnEmpty = false, allowEmpty = false } = props;
  let { doc } = props;
  if (!doc && docKey) {
    doc = getDoc(docKey);
  }
      ...
      <input
        ...
        onChange={e => {
          ...
          if (rawValue === '' || rawValue === '-') {
            // A fully-cleared field unsets the value so the consumer can drop
            // the config key. The per-dataset overrides emit null here, then
            // normalize it to a key delete so the field inherits the global
            // value. The trainer reads every field via kwargs.get(key, default),
            // so an absent key falls back to the default. With both flags off
            // we hold the previous value until a valid number is entered.
            if ((unsetOnEmpty || allowEmpty) && rawValue === '') {
              onChange(null);
            }
            return;
          }
